...
def player_info(url):
    ...
    if table:
        ...
        for row in table_foot.findAll('tr'):
            ...
            league = re.search(r'data-stat="lg_id">(.*?)</td>', playerData).group(1)
            if league != "TOT":
    ...
    player_entry = {'college_url': college_url,
                    'Leagues': leagues,
                    'NBA_fg%':fgpct ,
                    'NBA_g_played': games,
                    'NBA_ppg': ppg,
...
name_df = pd.concat([players_general_info, df], axis=1)
name_df.to_csv('namedLeaguePlayers_collegeURL.csv', encoding='utf-8')
